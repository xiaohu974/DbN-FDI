...
class CrossFusionBlock0(nn.Module):
    def __init__(self, dim, num_heads, bias, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.norm = nn.GroupNorm(dim // 2, dim)  # 归一化层
...
class CrossFusionBlock1(nn.Module):
    def __init__(self, dim, num_heads, bias, qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        #     nn.Conv2d(dim, dim // num_heads, 1, bias=False),
        #     nn.ReLU(inplace=True),
        #     nn.Conv2d(dim // num_heads, dim, 1, bias=False),
        # )    
        self.channel_attention = ChannelAttention(in_channels = dim, reduction = num_heads)

        self.fusion_conv = nn.Conv2d(dim * 3, dim, kernel_size=1, bias=False)
        self.norm = nn.GroupNorm(dim // 3, dim)  # 归一化层
        self.act = act_layer()  # 激活函数
...
class SAFNet(nn.Module):
    def __init__(self):
        ...
        # self.F_conv2 = ComplexConv2d(3, 3, 3, 1, 1)
    ...
    def forward(self, img):
        ...
        S_1 = self.spatial_1(self.S_conv1(img))  # 96
        F_1 = self.frequency_1(self.F_conv1(x_fft)) #96
        FU_1 = self.fusion_1(S_1, torch.fft.irfftn(F_1, dim=[-2, -1]))

        S_1_down = self.s_down_1(S_1)   # 96 - 48
        F_1_down = self.f_down_1(F_1)   # 96 - 48
        FU_1_down = self.s_down_1(FU_1) # 96 - 48

        S_2 = self.spatial_2(S_1_down)   # 48
        F_2 = self.frequency_2(F_1_down) # 48
        FU_2 = self.fusion_2(FU_1_down, S_2, torch.fft.irfftn(F_2, dim=[-2, -1])) # 48
        # FU_2 = self.fusion_2(S_2, torch.fft.irfftn(F_2, dim=[-2, -1])) # 48
        # FU_2 = self.fusion_2(FU_2, FU_1_down)

        S_2_down = self.s_down_2(S_2)   # 48 - 24
        F_2_down = self.f_down_2(F_2)   # 48 - 24
        FU_2_down = self.s_down_2(FU_2) # 48 - 24

        S_3 = self.spatial_3(S_2_down)
        F_3 = self.frequency_3(F_2_down)
        FU_3 = self.fusion_3(FU_2_down, S_3, torch.fft.irfftn(F_3, dim=[-2, -1]))
        # FU_3 = self.fusion_3(S_3, torch.fft.irfftn(F_3, dim=[-2, -1]))
        # FU_3 = self.fusion_3(FU_3, FU_2_down)

        S_3_down = self.s_down_3(S_3)   # 24 - 12
        F_3_down = self.f_down_3(F_3)   # 24 - 12
        FU_3_down = self.s_down_3(FU_3) # 24 - 12

        S_4 = self.spatial_4(S_3_down)
        F_4 = self.frequency_4(F_3_down)
        FU_4 = self.fusion_4(FU_3_down, S_4, torch.fft.irfftn(F_4, dim=[-2, -1]))
        # FU_4 = self.fusion_4(S_4, torch.fft.irfftn(F_4, dim=[-2, -1]))
        # FU_4 = self.fusion_4(FU_4, FU_3_down) # 12

        # S_out = self.conv2(S_4)   #3 
        # F_out = self.conv2(torch.fft.irfftn(F_4, dim=[-2, -1]))  #3
        # FU_out = self.conv2(FU_4) #3
                
        S_out = self.D_conv(S_4)   #3
        F_out = self.D_conv(torch.fft.irfftn(F_4, dim=[-2, -1]))  #3
        FU_out = self.D_conv(FU_4) #3
