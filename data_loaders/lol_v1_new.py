    ...
    def __getitem__(self, index):
        ...
        if self.mode == 'train':
            try:
                ...
                print(f"Error reading {data_lowlight_path}: {e}")
                # 您可以选择跳过这个图像或者返回一个默认值
                return None
            # data_lowlight = Image.open(data_lowlight_path).convert('RGB')
            data_highlight = Image.open(data_lowlight_path.replace('Underexposed', 'Normal_frames')).convert('RGB')
